...
namespace Z互联网
{
    public static class SocketHttpReqer
    {
        /// <summary>
        /// 使用socket得到网页html
        /// </summary>
        ...
        public static string GetHtml(string strUrl)
        {
            ...
            string strGetHeader = "GET " + GetRelativeUrl(strUrl) + " HTTP/1.1\r\nHost: " + strHost + "\r\nConnection: Close\r\n\r\n";
            byte[] getBytes = Encoding.ASCII.GetBytes(strGetHeader);
                int iTotalCount;
                byte[] responseBytes = GetHtmlOriginByte(strHost, getBytes, out iTotalCount); //得到网页
                string strResponse = Encoding.UTF8.GetString(responseBytes, 0, iTotalCount);
                //hasRelocated = HasRelocated(ref strResponse);
